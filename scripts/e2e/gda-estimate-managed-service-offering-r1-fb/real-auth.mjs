#!/usr/bin/env node
// GDA_ESTIMATE_MANAGED_SERVICE_OFFERING_R1_FB_REAL_AUTH
// Real local GoTrue tokens + PostgREST requests over public.save_estimate_from_wizard,
// against the forward-bridge migration's replayed function.
...
// request/record/fail idioms, using unique FB environment/dealer/user names.
// It copies no GYEON-order or R1-B fixture, table, RPC name, or evidence
// vocabulary beyond the shared repository schema every sibling harness in
...
const apiUrl = process.env.FB_API_URL;
const dbUrl = process.env.FB_DB_URL;
const anonKey = process.env.FB_ANON_KEY;
const serviceKey = process.env.FB_SERVICE_ROLE_KEY;

function fail(message) {
  throw new Error(`FB_REAL_AUTH_ERROR: ${message}`);
}

if (process.env.GDA_ESTIMATE_OFFERING_FB_DISPOSABLE_CONFIRM !== CONFIRM) fail('explicit disposable confirmation is missing');
if (!apiUrl || !dbUrl || !anonKey || !serviceKey) fail('FB_API_URL, FB_DB_URL, FB_ANON_KEY and FB_SERVICE_ROLE_KEY are required');
...
const runId = process.env.GDA_ESTIMATE_OFFERING_FB_SUFFIX ?? randomUUID().slice(0, 8);
const password = `FB-${randomUUID()}-aA1!`;
...
// The forward-bridge migration is applied after the local Supabase stack
// starts. Force PostgREST to reload only its local schema cache before any
// real HTTP assertion. Credentials stay in the child environment, never
// command args or evidence output.
try {
...
async function createUser(label) {
  const email = `fb-${label}-${runId}@example.invalid`;
  const created = await request('/auth/v1/admin/users', {
    method: 'POST', key: serviceKey, token: serviceKey,
    body: { email, password, email_confirm: true, app_metadata: { fb: true } },
  });
...
// the real Auth/PostgREST/RPC path (not raw SQL), against the forward-bridge
// function.
// ---------------------------------------------------------------------------
...
await serviceInsert('dealers', [
  { id: ids.dealerA, name: `FB Real Auth Dealer A ${runId}` },
  { id: ids.dealerB, name: `FB Real Auth Dealer B ${runId}` },
]);
...
function estimatePayload(idempotencyKey, category) {
  return {
    idempotencyKey,
    customer: {
      mode: 'new', name: 'FB Real Auth Customer', phone: '090-0000-0000', email: 'fb-real-auth@example.test',
    ...
    services: [{
      ...
      label: 'FB Real Auth Line', description: null,
...
const disabledFamilySave = await rpc('save_estimate_from_wizard', serviceKey, {
  p_dealer_id: ids.dealerA, p_actor_user_id: users.ownerA.id, p_payload: estimatePayload(`fbrealauthoff${runId}`.slice(0, 60), 'maintenance'),
});
...
const wrongDealerSave = await rpc('save_estimate_from_wizard', serviceKey, {
  p_dealer_id: ids.dealerB, p_actor_user_id: users.ownerA.id, p_payload: estimatePayload(`fbrealauthxd${runId}`.slice(0, 60), 'maintenance'),
});
...
const enabledFamilySave = await rpc('save_estimate_from_wizard', serviceKey, {
  p_dealer_id: ids.dealerB, p_actor_user_id: users.ownerB.id, p_payload: estimatePayload(`fbrealauthon${runId}`.slice(0, 60), 'maintenance'),
});
...
const unmanagedCategorySave = await rpc('save_estimate_from_wizard', serviceKey, {
  p_dealer_id: ids.dealerA, p_actor_user_id: users.ownerA.id, p_payload: estimatePayload(`fbrealauthcoat${runId}`.slice(0, 60), 'coating'),
});
