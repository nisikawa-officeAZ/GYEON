...
# GDA_ESTIMATE_MANAGED_SERVICE_OFFERING_R1_FB one-attempt evidence runner. Run
...
# strict-TAP-verification and evidence-sequencing pattern. It copies no
# GYEON-order or R1-B fixture, table, RPC name, or evidence vocabulary beyond
# the shared repository schema every sibling harness in this family already
# exercises.
#
# FB-SPECIFIC ADDITION: after replay, this script also extracts the live
# replayed function's canonical body and requires it to match the publicly
# committed accepted forward-bridge body hash. This is a public hash already
# recorded in Git-tracked governance documents, never private evidence.
...
EXPECTED_BRIDGE_BODY_SHA256="df49de1e6b8cf9767f32730cabdce5134b865678c63ab2d2a5c011400a2db7a6"

fail() {
  printf 'FB_EVIDENCE_ERROR: %s\n' "$1" >&2
...
[[ -n "$RUNTIME_DIR" ]] || fail "GDA_ESTIMATE_OFFERING_FB_RUNTIME_DIR is required"
[[ "$RUNTIME_DIR" =~ ^${RUNTIME_PARENT//\//\\/}/gda-estimate-offering-fb\.[0-9]{8}T[0-9]{6}Z-[a-z0-9]{6}$ ]] || fail "runtime path does not match the dedicated disposable pattern"
...
FB_API_URL="${API_URL:-${SUPABASE_URL:-}}"
FB_DB_URL="${DB_URL:-}"
FB_ANON_KEY="${ANON_KEY:-}"
FB_SERVICE_ROLE_KEY="${SERVICE_ROLE_KEY:-}"
[[ -n "$FB_API_URL" && -n "$FB_DB_URL" && -n "$FB_ANON_KEY" && -n "$FB_SERVICE_ROLE_KEY" ]] || fail "local status did not provide required endpoints and keys"
...
export FB_API_URL FB_DB_URL FB_ANON_KEY FB_SERVICE_ROLE_KEY
export GDA_ESTIMATE_OFFERING_FB_SUFFIX="${GDA_ESTIMATE_OFFERING_FB_SUFFIX:-$(basename "$RUNTIME_DIR" | sed 's/^gda-estimate-offering-fb\.//')}"

# ---------------------------------------------------------------------------
# FB-specific: prove the replayed function body matches the publicly
# committed accepted forward-bridge hash before any other evidence is
# gathered. This hash is already recorded in Git-tracked governance
# documents (not private evidence).
# ---------------------------------------------------------------------------

REPLAYED_BODY_SHA256="$(psql "$FB_DB_URL" -X -v ON_ERROR_STOP=1 -At -c "
select encode(sha256(convert_to(p.prosrc, 'UTF8')), 'hex')
  from pg_proc p
  join pg_namespace n on n.oid = p.pronamespace
 where n.nspname = 'public' and p.proname = 'save_estimate_from_wizard';
")"
log_cmd "psql extract replayed prosrc sha256" "$?"
[[ "$REPLAYED_BODY_SHA256" == "$EXPECTED_BRIDGE_BODY_SHA256" ]] || fail "replayed function body hash $REPLAYED_BODY_SHA256 does not match the accepted forward-bridge body hash $EXPECTED_BRIDGE_BODY_SHA256"
...
psql "$FB_DB_URL" -X -v ON_ERROR_STOP=1 -At > "$EVIDENCE_DIR/.applied-migrations.txt" <<'SQL'
...
psql "$FB_DB_URL" -X -v ON_ERROR_STOP=1 -At > "$EVIDENCE_DIR/schema-fingerprint.json" <<SQL
select jsonb_build_object(
  ...
  'replayed_body_sha256', '$REPLAYED_BODY_SHA256',
  'expected_bridge_body_sha256', '$EXPECTED_BRIDGE_BODY_SHA256'
)::text;
SQL
...
set +e
psql "$FB_DB_URL" -X -v ON_ERROR_STOP=1 -At > "$EVIDENCE_DIR/query-plans.txt" 2>&1 <<'SQL'
...
printf 'FB_EVIDENCE_CAPTURED=%s\n' "$EVIDENCE_DIR"
printf 'FB_CLASSIFICATION_PENDING=cleanup_then_manual_acceptance_review\n'
