#!/usr/bin/env node
// GDA_ESTIMATE_MANAGED_SERVICE_OFFERING_R1_FB_CONCURRENCY
// Implements exactly the two deterministic offering/save interleavings the
// governing directive requires, against the forward-bridge migration's
// replayed function. Every racing step below runs as a separate OS `psql`
// process; a third observer connection proves genuine simultaneous activity
// where required. No Promise-only simulation, no shared database session,
// and no single-connection pseudo-parallelism is accepted as proof.
//
...
// spawn/timeout/PID-proof idioms, using unique FB identifiers. It copies no
// GYEON-order or R1-B fixture, table, RPC name, or evidence vocabulary
// beyond the shared repository schema every sibling harness in this family
// already exercises.

import { spawn } from 'node:child_process';
import { randomUUID } from 'node:crypto';

const CONFIRM = 'I_UNDERSTAND_GDA_ESTIMATE_OFFERING_FB_IS_DISPOSABLE';
const dbUrl = process.env.FB_DB_URL;
if (process.env.GDA_ESTIMATE_OFFERING_FB_DISPOSABLE_CONFIRM !== CONFIRM) throw new Error('FB_CONCURRENCY_ERROR: explicit disposable confirmation is missing');
if (!dbUrl) throw new Error('FB_CONCURRENCY_ERROR: FB_DB_URL is required');
const db = new URL(dbUrl);
if (!['127.0.0.1', 'localhost', '::1'].includes(db.hostname)) throw new Error('FB_CONCURRENCY_ERROR: database URL must be loopback-only');
if (/supabase\.(co|in)|pooler\.supabase/.test(dbUrl)) throw new Error('FB_CONCURRENCY_ERROR: database URL must never resolve to a hosted Supabase host');

const suffix = process.env.GDA_ESTIMATE_OFFERING_FB_SUFFIX ?? randomUUID().slice(0, 8);
...
function psql(sql, label) {
  return new Promise((resolve) => {
    const child = spawn('psql', [dbUrl, '-X', '-v', 'ON_ERROR_STOP=1', '-At', '-c', sql], {
      stdio: ['ignore', 'pipe', 'pipe'], env: { ...process.env, PAGER: 'cat', PGAPPNAME: `gda-fb-${label}` },
    });
    let stdout = '';
    let stderr = '';
    let settled = false;
...
function startControlledHolder(dealerId, label) {
  const child = spawn('psql', [dbUrl, '-X', '-v', 'ON_ERROR_STOP=1', '-At'], {
    stdio: ['pipe', 'pipe', 'pipe'], env: { ...process.env, PAGER: 'cat', PGAPPNAME: `gda-fb-${label}` },
  });
  let stdout = '';
  let stderr = '';
  let settled = false;
  ...
  child.stdout.on('data', (chunk) => {
    stdout += chunk;
    const match = stdout.match(/FB_HOLDER_READY\|(\d+)/);
  ...
  child.on('close', (code) => {
    ...
    const marker = stdout.match(/FB_HOLDER_READY\|(\d+)/);
    resolveDone({
      label,
      code,
      stdout: stdout.trim(),
...
select 1 from public.document_sequences
 where dealer_id = '${dealerId}'
   ...
   and fiscal_year = 0
 for update;
select 'FB_HOLDER_READY|' || pg_backend_pid();
`);

  return {
    appName: `gda-fb-${label}`,
    ready,
    done,
    release() {
...
function estimatePayloadJson(idempotencyKey, category, configurationRevision = null) {
  const payload = {
    idempotencyKey,
    customer: {
      mode: 'new', name: 'FB Concurrency Customer', phone: '090-0000-0000', email: 'fb-conc@example.test',
    ...
    services: [{
      ...
      label: 'FB Concurrency Line', description: null,
...
values
  ('00000000-0000-0000-0000-000000000000','${owner1}','authenticated','authenticated','fb-conc-1-${suffix}@example.invalid','',now(),'{}','{}',now(),now()),
  ('00000000-0000-0000-0000-000000000000','${owner2}','authenticated','authenticated','fb-conc-2-${suffix}@example.invalid','',now(),'{}','{}',now(),now());
insert into public.dealers(id,name) values
  ('${dealer1}','FB Concurrency Dealer 1'),
  ('${dealer2}','FB Concurrency Dealer 2');
...
{
  const key = `fbdisablefirst${suffix}`.slice(0, 60);
...
{
  const key = `fbsnapshotfirst${suffix}`.slice(0, 60);
  await query(`
...
`, 'race2-fixture');
  ...
  const saveAppName = 'gda-fb-race2-save';
