...
# GDA_ESTIMATE_MANAGED_SERVICE_OFFERING_R1_B cleanup.
#
...
# scripts/e2e/gyeon-order-v3-c5c/cleanup.sh preserved-identifier /
# named-zero-row-proof / retained-hash-verification / manifest-last pattern.
# It copies no GYEON-order fixture, table, RPC name, or evidence vocabulary.
...
# failure. manifest.json -- the 16th and FINAL canonical artifact -- is
# generated only after evidence copy and exact runtime removal.
...
RETAINED_EVIDENCE_PARENT="${GDA_ESTIMATE_OFFERING_R1B_RETAINED_EVIDENCE_PARENT:-/Users/atsushinishikawa/Documents/Codex/runtime/gda-estimate-offering-r1b-evidence}"

fail() {
  printf 'R1B_CLEANUP_ERROR: %s\n' "$1" >&2
...
[[ -n "$RUNTIME_DIR" ]] || fail "GDA_ESTIMATE_OFFERING_R1B_RUNTIME_DIR is required and must not be blank"
case "$RUNTIME_DIR" in
...
[[ "$RUNTIME_DIR" =~ ^${RUNTIME_PARENT//\//\\/}/gda-estimate-offering-r1b\.[0-9]{8}T[0-9]{6}Z-[a-z0-9]{6}$ ]] || fail "runtime path does not match the exact dedicated disposable pattern"
...
run_fixture_teardown() {
  ...
  # dealers begin with the same 'R1B ' name prefix; both scripts' users use
  # an 'r1b-...@example.invalid' email pattern.
  DEALER_IDS_RAW="$(psql "$R1B_DB_URL" -X -v ON_ERROR_STOP=1 -At -c "select coalesce(string_agg(id::text, ','), '') from public.dealers where name like 'R1B %';")"
  ...
  USER_IDS_RAW="$(psql "$R1B_DB_URL" -X -v ON_ERROR_STOP=1 -At -c "select coalesce(string_agg(id::text, ','), '') from auth.users where email like 'r1b-%@example.invalid';")"
  ...
  ESTIMATE_IDS_RAW="$(psql "$R1B_DB_URL" -X -v ON_ERROR_STOP=1 -At -c "select coalesce(string_agg(e.id::text, ','), '') from public.estimates e join public.dealers d on d.id = e.dealer_id where d.name like 'R1B %';")"
...
SQL
  ...
  local report
  report="$(psql "$R1B_DB_URL" -X -v ON_ERROR_STOP=1 -At -F'|' -c "
...
if [[ -f "$EVIDENCE_DIR/supabase-status.env" ]]; then
  ...
  if [[ "$FIXTURE_ATTEMPTED" != "true" ]]; then
    ...
    log "attempt-started.txt absent; no R1-B fixture was created, so partial-schema fixture teardown is safely skipped"
  else
    R1B_DB_URL=""
    while IFS= read -r status_line; do
      if [[ "$status_line" == DB_URL=* ]]; then
        R1B_DB_URL="${status_line#DB_URL=}"
        R1B_DB_URL="${R1B_DB_URL#\"}"
        R1B_DB_URL="${R1B_DB_URL%\"}"
        R1B_DB_URL="${R1B_DB_URL#\'}"
        R1B_DB_URL="${R1B_DB_URL%\'}"
        break
...
lines = [
    "# GDA_ESTIMATE_MANAGED_SERVICE_OFFERING_R1_B summary", "",
...
[[ "$RUNTIME_DIR" == "$RUNTIME_PARENT"/gda-estimate-offering-r1b.* ]] || fail "final path safety check failed before removal"
set +e
rm -rf -- "$RUNTIME_DIR"
REMOVE_EXIT=$?
set -e
...
printf 'R1B_EVIDENCE_RETAINED=%s\n' "$RETAINED_DIR"
printf 'R1B_COPY_EXIT=%s\n' "$COPY_EXIT"
printf 'R1B_RUNTIME_REMOVAL_EXIT=%s\n' "$REMOVE_EXIT"
