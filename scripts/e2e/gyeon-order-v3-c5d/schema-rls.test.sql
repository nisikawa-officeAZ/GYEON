...
create temp table c5d_tables(name text primary key);
insert into c5d_tables(name) values
  ('product_orders'),
  ('product_order_items'),
...
select is(
  (select count(*) from c5d_tables t join pg_class c on c.relname = t.name join pg_namespace n on n.oid = c.relnamespace where n.nspname = 'public' and c.relkind = 'r'),
  19::bigint,
  '01 all nineteen R2-bound tables exist'
);

select is(
  (select count(*) from c5d_tables t join pg_class c on c.relname = t.name join pg_namespace n on n.oid = c.relnamespace where n.nspname = 'public' and c.relrowsecurity),
  19::bigint,
...
select is(
  (select count(*) from pg_policies where schemaname = 'public' and tablename in (select name from c5d_tables)),
  2::bigint,
  '05 exactly two policies exist across all nineteen tables'
);

select is(
  (select count(*) from pg_policies where schemaname = 'public' and tablename in (select name from c5d_tables) and cmd <> 'SELECT'),
  0::bigint,
...
select is(
  (select count(*) from information_schema.role_table_grants where table_schema = 'public' and table_name in (select name from c5d_tables) and grantee = 'anon'),
  0::bigint,
  '08 anon has no R2-bound table privilege'
);

select is(
  (select count(*) from information_schema.role_table_grants where table_schema = 'public' and table_name in (select name from c5d_tables) and grantee = 'authenticated' and privilege_type = 'SELECT'),
  2::bigint,
  '09 authenticated has SELECT only on the two order aggregates'
);

select is(
  (select count(*) from information_schema.role_table_grants where table_schema = 'public' and table_name in (select name from c5d_tables) and grantee = 'authenticated' and privilege_type in ('INSERT','UPDATE','DELETE','TRUNCATE','REFERENCES','TRIGGER')),
  0::bigint,
  '10 authenticated has no direct mutation privilege on any R2-bound table'
);

select is(
  (select count(*) from information_schema.role_table_grants where table_schema = 'public' and table_name in (select name from c5d_tables) and grantee = 'service_role' and privilege_type in ('SELECT','INSERT','UPDATE','DELETE')),
  76::bigint,
  '11 service_role has the bounded four table privileges on all nineteen tables'
);

select is(
  (select count(*) from information_schema.role_table_grants where table_schema = 'public' and table_name in (select name from c5d_tables) and grantee in ('authenticated','service_role') and is_grantable <> 'NO'),
  0::bigint,
...
insert into expected_columns(table_name, column_name, data_type, is_nullable, column_default) values
  ...
  ('gyeon_order_shipping_rule_versions','id','uuid','NO','pg_catalog.gen_random_uuid()'),
  ...
  ('gyeon_order_owner_review_events','id','uuid','NO','pg_catalog.gen_random_uuid()'),
  ...
  ('gyeon_order_external_evidence_v1','id','uuid','NO','pg_catalog.gen_random_uuid()'),
  ...
  ('gyeon_order_prepared_operations_v1','id','uuid','NO','pg_catalog.gen_random_uuid()'),
  ...
  ('gyeon_qualification_rule_versions','id','uuid','NO','pg_catalog.gen_random_uuid()'),
  ...
  ('gyeon_product_qualification_classification','id','uuid','NO','pg_catalog.gen_random_uuid()'),
  ...
  ('gyeon_dealer_qualification_mode_projection','id','uuid','NO','pg_catalog.gen_random_uuid()'),
  ...
  ('gyeon_order_qualification_snapshots','id','uuid','NO','pg_catalog.gen_random_uuid()'),
  ...
  ('gyeon_order_external_compensation_outbox','id','uuid','NO','pg_catalog.gen_random_uuid()'),
  ...
  ('gyeon_order_notification_outbox','id','uuid','NO','pg_catalog.gen_random_uuid()'),
  ...
  ('gyeon_product_order_offers_v3','id','uuid','NO','pg_catalog.gen_random_uuid()'),
...
insert into expected_table_grants(table_name, grantee, privilege_type)
  ...
  from (select name as t from c5d_tables) tables_
...
select is(
  (
    (select count(*) from (
      ...
      where table_schema='public' and table_name in (select name from c5d_tables)
    ) missing_from_actual)
    +
    (select count(*) from (
      select table_name, grantee, privilege_type from information_schema.role_table_grants
