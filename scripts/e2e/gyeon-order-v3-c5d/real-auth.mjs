...
const apiUrl = process.env.C5D_API_URL;
const dbUrl = process.env.C5D_DB_URL;
const anonKey = process.env.C5D_ANON_KEY;
const serviceKey = process.env.C5D_SERVICE_ROLE_KEY;
const psqlBin = process.env.GYEON_ORDER_V3_C5D_PSQL_BIN;

function fail(message) {
  throw new Error(`C5D_REAL_AUTH_ERROR: ${message}`);
}

if (process.env.GYEON_ORDER_V3_C5D_DISPOSABLE_CONFIRM !== CONFIRM) fail('explicit disposable confirmation is missing');
if (!apiUrl || !dbUrl || !anonKey || !serviceKey) fail('C5D_API_URL, C5D_DB_URL, C5D_ANON_KEY and C5D_SERVICE_ROLE_KEY are required');
if (!psqlBin) fail('GYEON_ORDER_V3_C5D_PSQL_BIN is required');
try {
  accessSync(psqlBin, fsConstants.X_OK);
} catch {
  fail('GYEON_ORDER_V3_C5D_PSQL_BIN must be an executable path');
}
...
const runId = process.env.GYEON_ORDER_V3_C5D_SUFFIX ?? randomUUID().slice(0, 8);
const password = `C5D-${randomUUID()}-aA1!`;
...
async function createUser(label) {
  const email = `c5d-${label}-${runId}@example.invalid`;
  const created = await request('/auth/v1/admin/users', {
    method: 'POST', key: serviceKey, token: serviceKey,
    body: { email, password, email_confirm: true, app_metadata: { c5d: true } },
  });
...
await serviceInsert('dealers', [
  { id: ids.dealerA, name: 'C5D Real Auth Dealer A', dealer_type: 'GYEON_DETAILER', status: 'active' },
  { id: ids.dealerB, name: 'C5D Real Auth Dealer B', dealer_type: 'GYEON_DETAILER', status: 'active' },
  { id: ids.dealerC, name: 'C5D Real Auth Dealer C (expired ordering)', dealer_type: 'GYEON_DETAILER', status: 'active' },
]);
...
await serviceInsert('gyeon_products', [{ id: ids.product, sku: `C5D-HTTP-${runId}`, product_name: 'C5D HTTP Product', category: 'coating', is_active: true }]);
...
await serviceInsert('gyeon_order_supply_projection', [{
  ...
  backorder_allowed: true, source_version: `c5d-http-${runId}`, observed_at: new Date().toISOString(),
}]);
...
const draft = await rpc('save_gyeon_order_v3_draft_rpc', users.staff.token, {
  ...
  p_draft_fields: { destination_kind: 'own_store', delivery_snapshot: { label: 'C5D Dealer A' } },
});
...
if (orderId) {
  const managerReview = await rpc('request_gyeon_order_v3_owner_review_rpc', users.manager.token, {
    ...
    p_expected_version: 1, p_idempotency_key: randomUUID(), p_note: 'C5D real Auth review',
  });
