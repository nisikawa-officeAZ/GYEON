#!/usr/bin/env node
// GYEON_ORDER_V3_C5D_CONCURRENCY
...
// This file implements exactly the ten races in plan section C5D-7.

import { spawn } from 'node:child_process';
import { randomUUID } from 'node:crypto';
import { accessSync, constants as fsConstants } from 'node:fs';

const CONFIRM = 'I_UNDERSTAND_GYEON_ORDER_V3_C5D_IS_DISPOSABLE';
const dbUrl = process.env.C5D_DB_URL;
const psqlBin = process.env.GYEON_ORDER_V3_C5D_PSQL_BIN;
if (process.env.GYEON_ORDER_V3_C5D_DISPOSABLE_CONFIRM !== CONFIRM) throw new Error('C5D_CONCURRENCY_ERROR: explicit disposable confirmation is missing');
if (!dbUrl) throw new Error('C5D_CONCURRENCY_ERROR: C5D_DB_URL is required');
if (!psqlBin) throw new Error('C5D_CONCURRENCY_ERROR: GYEON_ORDER_V3_C5D_PSQL_BIN is required');
try {
  accessSync(psqlBin, fsConstants.X_OK);
} catch {
  throw new Error('C5D_CONCURRENCY_ERROR: GYEON_ORDER_V3_C5D_PSQL_BIN must be an executable path');
}
const db = new URL(dbUrl);
if (!['127.0.0.1', 'localhost', '::1'].includes(db.hostname)) throw new Error('C5D_CONCURRENCY_ERROR: database URL must be loopback-only');
if (/supabase\.(co|in)|pooler\.supabase/.test(dbUrl)) throw new Error('C5D_CONCURRENCY_ERROR: database URL must never resolve to a hosted Supabase host');

const suffix = process.env.GYEON_ORDER_V3_C5D_SUFFIX ?? randomUUID().slice(0, 8);
...
function psql(sql, label) {
  return new Promise((resolve) => {
    const child = spawn(psqlBin, [dbUrl, '-X', '-v', 'ON_ERROR_STOP=1', '-At', '-c', sql], {
      stdio: ['ignore', 'pipe', 'pipe'], env: { ...process.env, PAGER: 'cat', PGAPPNAME: `gyeon-c5d-${label}` },
    });
    let stdout = '';
    let stderr = '';
    let settled = false;
...
async function raceTwo(raceName, sqlA, labelA, sqlB, labelB) {
  ...
  const observedActive = await query(
    `select count(distinct pid) from pg_stat_activity where application_name in ('gyeon-c5d-${labelA}','gyeon-c5d-${labelB}') and state = 'active'`,
    `${raceName}-observer`,
  );
...
async function backendPidOf(label) {
  return Number(await query(`select pid from pg_stat_activity where application_name = 'gyeon-c5d-${label}' order by backend_start desc limit 1`, `${label}-pid-lookup`));
}
...
values ('00000000-0000-0000-0000-000000000000','${ownerId}','authenticated','authenticated','c5d-concurrency-${suffix}@example.invalid','',now(),'{}','{}',now(),now());
insert into public.dealers(id,name,dealer_type,status) values ('${dealerId}','C5D Concurrency Dealer','GYEON_DETAILER','active');
...
insert into public.gyeon_products(id,sku,product_name,category,is_active) values ('${productId}','C5D-CONC','C5D Concurrency Product','coating',true);
...
values ('${productId}','CONFIGURED',100,0,0,100,true,'c5d-conc-v1',now());
...
values ('${orderId}','${productId}','C5D-CONC','C5D Concurrency Product',22000,1,15000,20000);
`;
}
...

{
  ...
  await new Promise((resolve) => setTimeout(resolve, 150));
  const observedActive = await query(
    "select count(distinct pid) from pg_stat_activity where application_name in ('gyeon-c5d-observer-hold-a','gyeon-c5d-observer-hold-b') and state = 'active'",
    'third-observer',
  );
