...
**ACCEPTED_WITH_CRITICAL_GAPS_DOCUMENTATION_ONLY**

この文書は、GYEON商品発注をDitailerOSへ統合する前の契約レビュー記録である。重大な未解決点が残っているため、DB、RPC、RLS、UI、決済、倉庫、通知の実装開始は承認しない。

このDraft PRで許可する変更は本書だけであり、runtime実装、既存Migration変更、新規Migration作成、DB接続、Supabase接続、テスト実行を含まない。
...
- 3万円送料無料の判定対象が税込／税抜、値引き前／後、送料を含む／含まないのどれかを決定する。
- 金額の丸め単位と税計算順序を固定する。
...
| --- | --- | --- |
| `AuthWindow` | `USE_EXISTING_DITAILEROS_PATTERN` | DitailerOSの実request scope、current dealer、role／disabled判定をauthorityとする。foundationの認証情報をそのまま採用しない。 |
| `PaymentPort` | `ADAPT_FOUNDATION_CONTRACT` | カードauthorization／captureの境界として利用し、PSP固有実装と秘密情報はserver側へ閉じ込める。 |
| `CaptureResultRepositoryPort` | `ADAPT_FOUNDATION_CONTRACT` | capture結果、idempotency、結果不明、再照合をDitailerOS DB契約へ適合させる。 |
| `WarehouseShippingDeps` | `ADAPT_FOUNDATION_CONTRACT` | GYEON Japan物流の送料マスター、倉庫受領、shipment authorityへ接続する。 |
| `WarehouseOrderLineInput` | `ADAPT_FOUNDATION_CONTRACT` | immutableな注文snapshotから倉庫入力を生成し、クライアント価格は渡さない。 |
| `ProductOrderItemSnapshot` | `ADAPT_FOUNDATION_CONTRACT` | 既存 `product_order_items` に必要なsnapshot列を新規Migrationで追加する前提で対応する。 |
| `captureAmountYen` | `ADAPT_FOUNDATION_CONTRACT` | 保存済み注文合計からserverが算出し、client指定額を拒否する。 |
| `captureIdempotencyKey` | `USE_EXISTING_DITAILEROS_PATTERN` | 完成済みpaymentsのcanonical idempotency／fingerprint設計を参考に、注文capture専用keyとして分離する。 |
| supplier availability | `NEW_GYEON_SPECIFIC_IMPLEMENTATION` | GYEONの商品販売状態、廃番、ランク、供給在庫、backorder許可を判定する。 |
| warehouse acceptance | `NEW_GYEON_SPECIFIC_IMPLEMENTATION` | 受領権限、受領時刻、受領後ロック、部分受領をDitailerOSの状態機械へ実装する。 |
| barcode validation | `NEW_GYEON_SPECIFIC_IMPLEMENTATION` | 送り状バーコードの一意性とscan結果をcapture開始条件として実装する。 |
| notification retry | `DEFER` | capture永続化と状態機械を確定した後、outbox／retry契約として別承認する。 |
...
次の判断は本書で推測せず、実装前の契約設計で決定する。

1. owner、manager、staff、readonlyのうち誰がカート作成、編集、submit、取消、再注文できるか。
2. 3万円送料無料を税込／税抜、値引き前／後、どの時点の金額で判定するか。
3. カードauthorizationを行う時点、authorization期限、再authorization、取消、返金の契約。
4. 6つのDB statusと4つのUI／TypeScript statusを統合した正規状態名と遷移表。
5. partial shipment、partial capture、backorder分割、取消可能数量の扱い。
6. 永続カートのowner、1 dealer当たりの個数、期限、引継ぎ、同時編集時の競合表示。
7. 送り状バーコードの発行元、形式、重複、誤scan、再scan、取消scanの扱い。
8. PSP、メール、倉庫API障害時のretry上限、dead-letter、管理者対応、照合手順。
9. GYEON Japan物流担当者／管理者をDitailerOS上でどのroleまたはcapabilityとして表現するか。

## 次の承認ゲート

次候補は**DB／RPC／RLS契約設計**である。ただし本Draft PRでは開始しない。

次フェーズを開始するには、Codexが本書を承認した後、少なくとも次を含む別の明示的な指示が必要である。

- 正規status transition表
- role／capability表
- server-owned金額計算
- atomic order RPC
...
- RLSとdealer非漏洩契約
- immutable snapshot
- authorization／capture／barcode／backorder／notificationの境界
