...
function Screen4_Estimate({ store, updateStore, dealerRank = 'certified' }) {
  ...
  /* ハイブリッド Screen 4:
     - Landscape 時: 左サイド縦タブ (200px) + 右コンテンツ (grid 2列)
     - Portrait 時: 上部 SegmentedControl + 下コンテンツ (単カラム)
     両方の DOM を出力し、CSS の (orientation) メディアクエリで表示切替。
     React の再マウントを避け、状態を保持したまま向きを変えられる。 */
  const contentBody = (
    <div>
  ...
  return (
    <div>
      <ScreenTitle step={4} title="見積" subtitle={`${activeCategories.length} 件のカテゴリを編集中。`} />

      {/* ─── Portrait レイアウト: 上部 SegmentedControl + 単カラム ───
          (orientation: portrait) のみ表示 */}
      <div className="s4-portrait-layout">
        <div style={{
      ...
      {/* ─── Landscape レイアウト: 左サイド縦タブ + 右コンテンツ ───
          (orientation: landscape) のみ表示 */}
      {/* CSS Grid の 1fr は minmax(auto, 1fr) と等価で、内容の min-content が
          親幅を超えると overflow する。minmax(0, 1fr) にして強制的に親幅内に
          収まるようにする (Grid の有名な罠を回避)。 */}
      <div className="s4-landscape-layout" style={{ display: 'grid', gridTemplateColumns: '200px minmax(0, 1fr)', gap: 20, alignItems: 'flex-start' }}>
        <nav style={{
          background: 'var(--ds-bg-card)',
          border: '1px solid var(--ds-line)',
          borderRadius: 'var(--ds-radius-xl)',
          ...
          <div className="ds-overline" style={{ padding: '8px 10px' }}>カテゴリ</div>
          {activeCategories.map(c => (
            <button key={c.id}
              onClick={() => setActiveTab(c.id)}
              style={{
                display: 'flex', alignItems: 'center', gap: 10,
                padding: '12px 14px', minHeight: 52,   /* Tablet タッチ拡大 */
                background: activeTab === c.id ? 'var(--ds-primary-tint-40)' : 'transparent',
                border: `1px solid ${activeTab === c.id ? 'var(--ds-primary-tint-60)' : 'transparent'}`,
                borderRadius: 10, cursor: 'pointer',
                color: activeTab === c.id ? '#fff' : 'var(--ds-text-muted)',
                fontFamily: 'var(--ds-font-sans)',
                fontSize: 14, fontWeight: activeTab === c.id ? 600 : 500,
                textAlign: 'left',
                transition: 'all var(--ds-dur-fast) var(--ds-ease-out)',
              }}>
              <Icon name={c.icon} size={18} />
              <span style={{ flex: 1 }}>{c.label}</span>
              {store.categoryDone?.[c.id] && <Icon name="check" size={14} style={{ color: 'var(--ds-green-400)' }} />}
            </button>
          ))}
        </nav>
        <div>{contentBody}</div>
...
function WindowFilmSection({ store, updateStore }) {
  ...
  return (
    ...
    >
      ...
      <div style={{
        display: 'grid',
        /* Desktop: 2 列 (幅広なので窓 3 個ペアで並ぶ)。Tablet/Mobile では別途 override。 */
