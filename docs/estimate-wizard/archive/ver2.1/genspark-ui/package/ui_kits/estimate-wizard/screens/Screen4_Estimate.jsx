...
function Screen4_Estimate({ store, updateStore, dealerRank = 'certified' }) {
  ...
  return (
    <div>
      <ScreenTitle step={4} title="見積" subtitle={`${activeCategories.length} 件のカテゴリを編集中。左サイドから切替できます。`} />

      {/* minmax(0, 1fr) で子要素が親幅を超えて overflow するのを防ぐ (Grid の 1fr 罠)
          PC 版でも視覚的に問題が出にくいだけで、同じ潜在バグを持っていた。 */}
      <div style={{ display: 'grid', gridTemplateColumns: '200px minmax(0, 1fr)', gap: 20, alignItems: 'flex-start' }}>
        {/* 左サイド縦タブ (Ver2.0 §6 - Desktop)
            top を共通トークン --ds-sticky-top に統一 → 右の合計パネルと
            同じ Y 座標で追従する（3 ウィンド上端揃い + 一体追従）。 */}
        <nav style={{
          ...
          {activeCategories.map(c => (
            <button key={c.id}
              ...
              style={{
                ...
                fontSize: 13, fontWeight: activeTab === c.id ? 600 : 500,
                textAlign: 'left',
                transition: 'all var(--ds-dur-fast) var(--ds-ease-out)',
              }}>
              <Icon name={c.icon} size={16} />
              <span style={{ flex: 1 }}>{c.label}</span>
              {store.categoryDone?.[c.id] && <Icon name="check" size={12} style={{ color: 'var(--ds-green-400)' }} />}
            </button>
          ))}
        </nav>
