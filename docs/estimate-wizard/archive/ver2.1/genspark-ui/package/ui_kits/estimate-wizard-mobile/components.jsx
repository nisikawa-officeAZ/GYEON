...
/* ═══════════════════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════════════════
   スマホ (<768px) 専用コンポーネント。iOS/Android のネイティブ体験に近い形。 */

/* ── BottomSheet ──────────────────────────────────────────────────────
   下からスライドアップするシート。iOS/Android の Action Sheet に相当。
   Screen 4 の詳細展開、内訳表示、ステッパーシート等、スマホ主要 UX。
   スワイプダウンで閉じる (現実装はハンドルとタップで close)。 */
function BottomSheet({ open, onClose, title, subtitle, icon, children, actions, height = 'auto', showHandle = true }) {
  if (!open) return null;
  return (
    <div
      onClick={onClose}
    ...
    >
      <div
        ...
        style={{
          ...
          animation: 'ds-slide-in-bottom 0.28s var(--ds-ease-out)',
      ...
      >
        ...
        {showHandle && (
          <div style={{ display: 'flex', justifyContent: 'center', paddingTop: 4, paddingBottom: 12 }}>
            <div style={{ width: 40, height: 4, borderRadius: 2, background: 'var(--ds-line-strong)' }} />
          </div>
        )}
        {(title || icon) && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              {title && <h2 className="ds-title" style={{ margin: 0, fontSize: 17 }}>{title}</h2>}
              {subtitle && <p className="ds-caption" style={{ margin: '2px 0 0' }}>{subtitle}</p>}
            </div>
            <button onClick={onClose} style={{
              width: 36, height: 36, borderRadius: 10,
              background: 'var(--ds-bg-elevated)', border: '1px solid var(--ds-line)',
              color: 'var(--ds-text-muted)', cursor: 'pointer',
              display: 'grid', placeItems: 'center', flexShrink: 0,
            }}>
              <Icon name="x" size={18} />
        ...
        {actions && (
          <div style={{
            ...
            paddingTop: 16, borderTop: '1px solid var(--ds-line)',
          }}>
            {actions}
          </div>
        )}
...
/* ── NativeCameraButton ──────────────────────────────────────────────
   iOS/Android のネイティブカメラを起動する input[type=file capture]。
   ボタン表示上は SelectButton や Button の見た目を継承 (透明 input を重ねる)。
   accept="image/*" + capture="environment" で背面カメラ起動。 */
function NativeCameraButton({ children, onCapture, variant = 'primary', size = 'lg', icon = 'camera', style = {}, full = false }) {
  ...
  return (
    <label style={{ display: full ? 'block' : 'inline-block', width: full ? '100%' : 'auto', cursor: 'pointer' }}>
      <input
        ref={inputRef}
        type="file"
        accept="image/*"
...
/* ── CategoryGridCard ────────────────────────────────────────────────
   Screen 4 のカテゴリカード。タップで詳細ボトムシートを開くタイル UI。
   iOS Springboard に近い視覚的な触感 (アイコン中央・タイトル下・状態バッジ)。 */
function CategoryGridCard({ category, selected, hasContent, onClick, style = {} }) {
  return (
    <button
      onClick={onClick}
      aria-pressed={!!selected}
      style={{
        ...
        gap: 8,
        /* Mobile 想定 393-430px 幅で 2 列時、1 カード = 約 175-200px。
           高さは 140-160px に固定して縦潰れせず、幅広画面でも間延びしない。 */
        height: 140,
        padding: 12,
        background: selected ? 'var(--ds-primary-tint-40)' : 'var(--ds-bg-card)',
        border: `1px solid ${selected ? 'var(--ds-primary-tint-60)' : 'var(--ds-line)'}`,
        borderRadius: 'var(--ds-radius-xl)',
        cursor: 'pointer',
        color: 'var(--ds-text-primary)',
        ...
        transition: 'all var(--ds-dur-fast) var(--ds-ease-out)',
        boxShadow: selected
          ? 'inset 0 0 0 1px rgba(59,130,246,0.15), 0 4px 12px -4px rgba(29,78,216,0.3)'
          : 'var(--ds-shadow-md), var(--ds-hi-inner)',
    ...
    >
      ...
      <div style={{
        ...
        background: selected ? 'rgba(59,130,246,0.20)' : 'var(--ds-bg-elevated)',
        display: 'grid', placeItems: 'center',
...
/* ── OCRReviewSheet ─────────────────────────────────────────────────
   Mobile 版 OCR レビュー: ネイティブカメラで撮影後、AI-OCR 結果を
   BottomSheet で表示。ユーザーが確認・修正して適用。
   PC/Tablet の OCROverlay/OCRSlidePanel の Mobile 版。 */
function OCRReviewSheet({ open, onClose, onApply, target = '車検証' }) {
  const [phase, setPhase] = useState('scanning'); // scanning | review
  useEffect(() => {
    if (open) {
      setPhase('scanning');
      /* Mock: 実運用ではファイルを OCR API に送るが、ここでは 1.5 秒後に review へ */
      const t = setTimeout(() => setPhase('review'), 1500);
      return () => clearTimeout(t);
    }
  }, [open]);
  return (
    <BottomSheet
      ...
      onClose={onClose}
      title={`${target} OCR 読み取り`}
      subtitle={phase === 'scanning' ? 'AI が読み取り中...' : '読み取り結果を確認'}
      icon={phase === 'scanning' ? 'loader-2' : 'scan-line'}
    >
      {phase === 'scanning' ? (
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
            <Button variant="ghost" full onClick={onClose}>キャンセル</Button>
            <Button variant="primary" full icon="check" onClick={() => { onApply && onApply(); onClose(); }}>適用</Button>
          </div>
        </div>
      )}
    </BottomSheet>
...
/* ── QRScanSheet (Mobile) ────────────────────────────────────────
   LINE QR: ネイティブカメラで撮影後、認識結果を BottomSheet で確認。 */
function QRScanSheet({ open, onClose, onApply }) {
  useEffect(() => {
    if (open) {
      /* Mock: 実運用ではカメラで QR を認識、ここでは 1.2 秒後にダミー ID を返す */
      const t = setTimeout(() => {
        onApply && onApply('yamada_taro_line');
        onClose();
      }, 1200);
      return () => clearTimeout(t);
    }
  ...
  return (
    <BottomSheet open={open} onClose={onClose} title="LINE QR 認識中" icon="qr-code" showHandle={false}>
      <div style={{ padding: '30px 20px', textAlign: 'center' }}>
        <div style={{ display: 'inline-block', animation: 'ds-spin 1.2s linear infinite', color: 'var(--ds-primary-400)', marginBottom: 16 }}>
          <Icon name="loader-2" size={44} />
        </div>
        <div style={{ fontSize: 14, color: 'var(--ds-text-body)' }}>QR コードを認識しています...</div>
      </div>
    </BottomSheet>
  );
}

/* Global helpers to be attached (Mobile: adds BottomSheet / NativeCameraButton / CategoryGridCard / OCRReviewSheet / QRScanSheet) */
Object.assign(window, {
  ...
  BottomSheet, NativeCameraButton, CategoryGridCard,
  OCRReviewSheet, QRScanSheet,
});
