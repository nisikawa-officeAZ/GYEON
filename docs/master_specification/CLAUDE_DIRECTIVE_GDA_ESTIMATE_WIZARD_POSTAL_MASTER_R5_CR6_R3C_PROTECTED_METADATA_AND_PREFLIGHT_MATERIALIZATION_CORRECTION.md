# CLAUDE DIRECTIVE — GDA Estimate Wizard Postal Master R5 CR6-R3C Protected Metadata and Preflight Materialization Correction

## 1. Status, authority, and supersession

```yaml
phase: GDA_ESTIMATE_WIZARD_POSTAL_MASTER_R5_CR6_R3C_PROTECTED_METADATA_AND_PREFLIGHT_MATERIALIZATION_CORRECTION
marker: GDA_ESTIMATE_WIZARD_POSTAL_MASTER_R5_CR6_R3C_PROTECTED_METADATA_AND_PREFLIGHT_MATERIALIZATION_CORRECTION_DIRECTIVE_V1
...
fixed_predecessor_head: 33c1ec65292e2d7112f73130a2446408541e96b1
fixed_predecessor_tree: f373ec66ba47e8e68db7a399d451e831e0ce23e0
base: main
...
supersedes: GDA_ESTIMATE_WIZARD_POSTAL_MASTER_R5_CR6_R3B_GIT_ACQUISITION_AND_DELEGATION_CORRECTION_DIRECTIVE_V1
rediagnosis_instruction: https://github.com/nisikawa-officeAZ/GYEON/pull/67#issuecomment-5553972075
codex_acceptance: https://github.com/nisikawa-officeAZ/GYEON/pull/67#issuecomment-5554059993
```

This directive fully supersedes CR6-R3B. It retains the exact aggregate byte
contract, public-to-core delegation boundary, protected monthly-invoice digest
provenance, bounded CLI-discovery evidence contract, and two-file future
implementation allowlist. It closes the two remaining defects: complete
protected-path Git metadata acquisition and local isolated materialization in
both explicit modes.
...
The one-time CR6-R3B tool-disabled diagnosis returned
`CHANGES_REQUIRED_DIRECTIVE`. MacBook Codex independently accepted both
findings. No adapter implementation may begin until both are corrected:

1. the closed Git argv contract enumerated only `supabase/migrations/` and
   therefore could not supply mode/blob evidence for the two protected paths
   outside that directory; and
2. the 112-blob isolated materialization transaction was restricted to
   `execute-once`, while `preflight-only` also required an independently
   inspected 112-file actual workdir and monthly batch-header provenance.

During CR6-R3C authoring, MacBook Codex rejected an initial recursive
three-path `ls-tree` correction because the fixed tree returns 117 records:
113 formal top-level SQL files, two nested `DRAFT_DO_NOT_APPLY` files, and the
two external protected paths. This is not a third Claude finding. It is an
authoring-time contract correction: Section 8 instead uses one non-recursive
migration-subtree response of 114 direct children and one separate response of
exactly two external protected paths.
...
1. `preflight-only`: local-only preflight. It may create, materialize,
   inspect, and securely delete one fresh isolated runtime tree, but it must
   never construct or start a linked Supabase command, contact a hosted target,
   burn an execution attempt, or create retained hosted evidence; and
...
protected contents. A later separately approved invocation of either explicit
mode may stream the fixed monthly-invoice Git blob directly into an isolated
workdir without displaying, logging, retaining, or transmitting its SQL text.
The protected LINE blob is never requested, materialized, or applied.
...
1. fixed branch, project ref, counts, exclusion, protected metadata, monthly
   attestation, expected aggregate, and the separately accepted CR6-R3C
...
Parse the two responses independently and never concatenate them before their
own count and shape checks.

The fixed migration-subtree response must contain exactly 114 unique direct
child records: exactly 113 filename-only `.sql` blobs with mode `100644`, plus
exactly one `DRAFT_DO_NOT_APPLY` tree record with mode `040000`, type `tree`,
and fixed object id `b6b9b1bd0cefedd0a08a40ef7c2c55c4fa5f4018`. The command is
non-recursive: neither file below that tree may be returned. Reject a slash in
any returned child name, any other tree, any other path or suffix, duplicate,
missing, extra, wrong-mode, wrong-type, wrong-object, control byte, invalid
UTF-8, or malformed record. Prefix each accepted SQL filename internally with
`supabase/migrations/`, then require the 113 reconstructed paths in canonical
byte order before manifest construction.

The external-protected response must contain exactly two unique records, in
the requested order, for exactly the two named non-migration protected paths.
Both must have mode `100644`, type `blob`, their fixed object ids, no control
bytes, and valid UTF-8. Reject any duplicate, missing, extra, reordered,
wrong-mode, wrong-type, wrong-object, or malformed record. Across the accepted
migration and external responses, verify the fixed mode/blob identity of all
four protected paths.
...
order. It must not request either protected blob. For both `preflight-only` and
`execute-once`, after that mode's separate invocation authorization, run one
local-only materialization transaction requesting exactly the 112 staged blob
...
- `preflight-only` may perform the local 112-blob materialization and actual
  inspection only. It cannot construct or spawn migration-list or migration-up,
  contact a hosted target, burn an execution attempt, or retain hosted evidence;
  it must securely delete the isolated tree before returning.
...
3. malformed NUL framing, mode/type/object/path/count/order/UTF-8 drift,
   duplicate/missing/extra `ls-tree` records, either missing non-migration
   protected path, recursion into either `DRAFT_DO_NOT_APPLY` file, a missing
   or changed fixed draft-tree record, any 115th migration-subtree record, or
   any third external-protected record fails closed;
...
11. both modes perform the exact local 112-blob materialization and independent
    actual inspection; `preflight-only` then deletes the isolated tree and never
    constructs/spawns a linked command, burns an attempt, contacts hosted state,
    or retains hosted evidence; unsafe roots, wrong fixed identity,
    dirty/diverged Git, burned execution attempt, ledger mismatch, and evidence
...
`GDA_ESTIMATE_WIZARD_POSTAL_MASTER_R5_CR6_R3C_READ_ONLY_DIAGNOSIS_RESULT_V1`
