...
|-------|-------|
| **Version** | 1.0 |
| **Status** | Canonical (⚠️ OD-2,3,4,5,6,7,9,10,15 pending — pricing/ID decisions) |
| **Last Updated** | 2026-06-25 |
| **Canonical Source** | `gyeon_flow.json` (meta.version 2024-06) |
| **Related Documents** | `04_SETTINGS_WORKFLOW.md`, `05_DATABASE_REQUIREMENTS.md`, `08_UI_REQUIREMENTS.md`, `11_CANONICAL_RULES.md`, `OPERATOR_DECISIONS.md` |

> **Derived directly from `gyeon_flow.json`** (`meta.version = 2024-06`).
> This document is canonical. It documents every screen, step, condition, calculation, and transition exactly as defined in the JSON. The implementation (`src/components/estimates/EstimateWizard.tsx`) must conform to this.
...
**Rule:** Single-category selection is not prohibited, but multi-category selection is the primary use case. All combinations are valid. No combination is blocked.
...
- **dealer_rate** (number %, shown only when `is_dealer === true`, default **100**)

**Special features:**
- Existing-customer picker modal (search the customer store by name/phone).
...
**Input modes:** (a) manual 8-size buttons; (b) dimension input (全長×全幅×全高 mm → auto-classify).

**Body size sharing rule:** When `coating` and `ppf` are both selected, STEP2 runs **once**. The same `sizeKey` is applied to both coating price calculation and PPF plan price lookup.
...
> ⚠️ **Known inconsistency:** body sizes use `XL`/`XXL`, but PPF `plan_prices` use `LL+`. These key sets do not align. Flagged for operator resolution in `09_PHASE_STATUS.md`.
...
**Layer modes:** `none` (single layer) / `2layer` (main + topcoat) / `3layer` (main + topcoat×2).

**Topcoat options by context:**
| context | allowed topcoats |
|---------|-----------------|
| detailer_2layer | ONE EVO, PURE EVO, MOHS EVO, CanCoat EVO |
| detailer_3layer | CanCoat EVO |
| certified_normal_2layer | ONE EVO, PURE EVO, MOHS EVO, CanCoat EVO, CanCoat EVO PRO |
...
| syncro_2layer | MOHS EVO |
| syncro_3layer | 不可 (blocked) |

**Topcoat base prices (M size, ¥):**
one-evo 15,000 · pure-evo 20,000 · mohs-evo 25,000 · cancoat-evo 18,000 · cancoat-evo-pro 25,000 · infinit1 130,000 · infinit2 160,000 · infinit-t1 40,000 · infinit-t2 50,000.
...
**Display condition:** `ppf` selected.
**Sub-steps:** ①plan ②film type ③vehicle rank ④front-glass option (optional) ⑤single parts (optional).

**Plans & base prices (¥) by size:**
...
- std: anything else / unknown (manual selection)
...
**Menu structure:** 5 fixed slots A–E (name/price editable in settings; empty name = hidden in wizard). Plus free items (free name/price input, add/remove rows).
**Formula:** `Σ(selected menu price) + Σ(free item price)`

### 4.8 Car wash step (`screen-step-carwash`)

**Display condition:** `carwash` selected.
**Default menus (¥):** cw-hand 3,000 · cw-polish 5,000 · cw-coat 8,000 · cw-wax 5,000 · cw-vacuum 2,000.
Settings allow unlimited add/edit/delete.
...
**Coupon system:** 5 fixed slots. Defaults: 新規ご来店 ¥5,000 · リピーター ¥3,000 · 紹介特典 ¥5,000 · キャンペーン ¥10,000 · スタッフ ¥3,000. Multiple selectable; summed into `couponDiscount`.
...
**Tax:** `(subtotal - 各種割引) × tax_rate / 100`

**Actions:**
| Label | Action | Note |
|-------|--------|------|
| 保存して完了 | `saveAndComplete()` | save estimate + sync |
| LINE転送 | `shareToLine()` | share estimate text via LINE |
| PDF保存 | `savePdf()` | generate PDF download |

---

## 5. `currentEstimate` data structure (canonical)

Held in memory while building an estimate. Must be persisted on save.

| Field | Type | Notes |
|-------|------|-------|
| customer | object | name, phone, address |
| customer_id | integer\|null | link to existing customer record |
| car | object | maker, model, year, color, plate |
| size | string\|null | SS\|S\|M\|ML\|L\|LL\|XL\|XXL |
| coating | string\|null | selected coating ID |
| layerMode | string | none\|2layer\|3layer |
| layerCount | integer | 0\|2\|3 |
| topcoat2 | string\|null | 2nd layer topcoat ID |
| topcoat3 | string\|null | 3rd layer topcoat ID |
| topcoats | array | [topcoat2, topcoat3] summary |
| options | array | selected option IDs |
| ppf | object | plan, filmType, vehicleRank, frontGlass, parts[], doorCupCount(1–6), sizeKey, subtotal |
| windowFilm | object | parts[], grade, partColors{}, subtotal |
| maintenance | object | items[], freeItems[], subtotal |
| carWash | object | items[], subtotal |
| roomCleaning | object | parts[], condition, freeItems[], subtotal |
| otherWork | object | items[], subtotal |
| categories | array | selected category IDs |
| notes | string | 備考 |
| couponDiscount | number | sum of applied coupons |
| appliedCoupons | array | applied coupon names |
| discountAmount | number | manual discount amount |
| is_dealer | boolean | 業販フラグ |
| dealer_rate | number | % (default 100) |
| dealer_discount | number | calculated dealer discount |
| created_at | string | ISO8601 |

> This canonical structure is the contract the implemented estimate tables/types must satisfy. See `05_DATABASE_REQUIREMENTS.md`.

---

## 6. Persistence (as defined in canonical JSON — legacy runtime reference)

The canonical JSON specifies these IndexedDB stores (for reference; current implementation uses Supabase):
...
> ⚠️ The current implementation uses **Supabase (PostgreSQL)**, not IndexedDB/Cloudflare D1. Business meaning (estimates/customers/maintenance persist; per-dealer isolation) carries over; the mechanism is replaced. See `05_DATABASE_REQUIREMENTS.md` §4 and `11_CANONICAL_RULES.md` §6.
