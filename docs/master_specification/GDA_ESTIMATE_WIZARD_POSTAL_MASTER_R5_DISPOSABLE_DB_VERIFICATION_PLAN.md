...
- 本closeout invocationはRead/Editのみを使用し、当時はGit未staged・未commit・未push、DB/runtime未実行のままCodexの静的レビュー待ちだった。その後、clean replacement、identity repair、evidence-detail repairはそれぞれ別ゲートでcommit・通常pushされた。
- PR `#67`のR5 source/harness identity repairはcommit `b8687b6f3b94c88d7abea76f7ffc7612d89acf67`として通常pushされた。最初のowner-approved runtime attempt（suffix `20260903T085447Z-5af594`）ではDB契約自体は全て通過したが、`import-resume.mjs`が成功時にも失敗理由をdetailへ固定出力する証拠矛盾をCodexが検出し、`CHANGES_REQUIRED_HARNESS`とした。runtimeとcontainerは削除され、証拠は保持した。同じsuffixは再利用していない。
- 証拠detailの修正はcommit `1ea1b5f2e3970184610721f261607b5e3f64656c`、tree `573c1db1526331c0cd569622da2a68168b118786`としてPR `#67`へ通常pushされ、VercelとVercel Preview CommentsはPASSした。
- 修正後HEADを固定した2回目のowner-approved runtime attempt（suffix `20260903T090318Z-54ce39`）は`GDA_POSTAL_R5_DISPOSABLE_DB_PASS`。fresh/import両laneでfull-chain migration replay、target migration ledger count `1`、pgTAP `75/75 + 20/20`、real Auth/PostgREST `9/9`、import phase 1 `3/3`、phase 2 `25/25`、DB lint error `0`、secret scan clean、source-contract drift `0`を確認した。両project stopはexit `0`、burnなし、runtime/containerは完全削除済みである。
- 最終retained evidenceは`/Users/atsushinishikawa/Documents/Codex/runtime/gda-postal-r5-evidence/gda-postal-r5.20260903T090318Z-54ce39`、aggregate `manifest.json` SHA-256は`92af4ed809cd42476810a00786b0c6d7d86888186f23fc761fde2c9f5c2cff31`。Development、staging、production、hosted Supabase、providerには接続していない。
- R5 PASS記録commit `4ce73f99bca0c43617007d90125da209fb2ea699`はPR `#67`へ通常pushされ、VercelおよびVercel Preview CommentsはPASSした。
- その後、ownerがDevelopment migration-historyと実DB構造の読み取り専用preflightを個別承認した。環境正本を管理APIで照合し、Developmentは`DealerOS-Dev` / ref `fbieiotihlmpfzybowbt` / region `ap-northeast-2` / `ACTIVE_HEALTHY`と確認した。StagingとProductionは識別用管理メタデータのみを確認し、両DBには接続していない。
- Developmentのremote migration ledgerは`000`と`001`の2件だけだが、実DBには少なくともlocal migration `038`、`047`、`068`、`095`由来のテーブルが存在する。さらに`096`は`gyeon_ai_usage_log.response_ms`だけが存在し`public.pg_version()`は不存在、`097`から`102`までの代表的schema markerは存在、`103`の`public.wizard_catalog_items`と`public.wiz_is_any_active_member()`は不存在である。これは未適用の一直線なsuffixではなく、手動適用・部分適用・ledger未登録が混在するschema driftである。
- R5固有の`private` schema、3 tables、lookup/import 7 RPCはすべて不存在であり、対象migrationが依存する`public.wiz_is_any_active_member()`も不存在である。このため判定は`GDA_POSTAL_R5_DEVELOPMENT_READ_ONLY_PREFLIGHT_BLOCKED_SCHEMA_DRIFT`。現Developmentへのtarget migration単独適用、bulk `db push`、migration history repair、実CSV importはいずれも未承認かつ禁止のままである。
- `ENVIRONMENT_LEDGER.md` Gate B-R3Aの既決定どおり、現Developmentはaccepted cutoverまでread-onlyを維持し、in-place history relabelを行わずclean replacement Developmentを使用する。project creation、data copy、migration replay/apply、cutover、旧project retirementはそれぞれ別owner gateを必要とする。
