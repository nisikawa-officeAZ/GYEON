...
The following supporting documents govern the current GYEON Detailer Agent completion sequence. They do not replace the frozen v2.0 architecture; they control current priority, phase authorization, acceptance evidence, and result recording.
...
| Dealer Order Formal Decision V3 | `SPEC_GYEON_ORDER_001_DEALER_ORDER_FORMAL_DECISION_V3.md` | Owner-ratified; locally committed at `37eb6d4`; push pending | Binding dealer-order business/UI decisions, supersession boundaries, unresolved implementation decisions, and acceptance contract. Runtime implementation remains separately unauthorized. |
| GenSpark Dealer Order UI Production Request V3 | `SPEC_GYEON_ORDER_001_GENSPARK_UI_PRODUCTION_REQUEST_V3.md` | Owner-ratified; locally committed at `37eb6d4`; push pending | Exact no-invention production request for PC, tablet, and mobile dealer-order UI, states, roles, payments, warehouse calendar, notifications, and handoff evidence. |
