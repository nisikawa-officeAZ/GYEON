-- GDA-ESTIMATE-POST-TAX-ADJUSTMENT-R1 — forward-only replacement of the atomic
-- estimate-to-invoice conversion's MONEY BLOCK only.
--
-- Owner-ratified rule (identical to calculateEstimateTotals / calculateInvoiceTotals):
--   tax_amount = floor(subtotal * tax_rate / 100)            -- tax on the FULL subtotal
--   applied    = clamp(requested_discount, 0, subtotal + tax) -- post-tax gross clamp
--   total      = subtotal + tax_amount - applied
--
-- Everything else is byte-faithful to the accepted definition in
-- 20260913163712_atomic_estimate_invoice_conversion.sql (which remains untouched):
-- authorization gate, dealer scoping, replay/conflict/idempotency behavior,
-- single-transaction atomicity, numbering, NaN/Infinity/2^53 guards, security
-- invoker, empty search_path, and the revoke/grant surface. This migration
-- contains NO table/column change, NO data UPDATE, and NO backfill: existing
-- rows (draft or issued) are never rewritten.
create or replace function public.create_invoice_from_estimate_atomic(
...
begin
  ...
  -- GDA-ESTIMATE-POST-TAX-ADJUSTMENT-R1: same post-tax rule and Math.floor tax as
  -- calculateInvoiceTotals. Tax is computed on the FULL subtotal; the document
  -- discount is clamped to the gross [0, subtotal + tax] and subtracted AFTER tax.
  -- Keep the original stored discount field, as the existing conversion does.
  v_tax := floor(v_subtotal * v_tax_rate / 100::double precision);
  v_x := least(greatest(0::double precision, v_discount), v_subtotal + v_tax);
  v_total := v_subtotal + v_tax - v_x;
