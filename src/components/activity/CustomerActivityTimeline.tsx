...
import { getActivityLogsByCustomer } from "@/lib/activity/activity-log";
...
export default function CustomerActivityTimeline({ customerId }: Props) {
  ...
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    getActivityLogsByCustomer(customerId).then((data) => {
      if (!cancelled) {
        setLogs(data);
        setLoading(false);
      }
  ...
  if (logs.length === 0) {
    return <p className="text-xs text-slate-500">まだアクティビティはありません</p>;
  }
          ...
          {logs.map((log) => (
            <li key={log.id} className="relative flex gap-3 pl-5">
              ...
              <div className="flex-1 min-w-0">
                {/* Top row: entity badge + action label */}
