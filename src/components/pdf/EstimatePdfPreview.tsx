// A4-style printable estimate preview.
// Accepts real EstimateDB data — no mock values.
        ...
        <div className="bg-white text-gray-900 rounded-lg shadow-lg mx-auto max-w-3xl p-8 sm:p-12">
          ...
          <div className="flex items-start justify-between mb-8">
            ...
            <div className="text-right">
              ...
              <p className="text-sm text-gray-500">{estimate.estimate_no}</p>
              <p className="text-xs text-gray-400 mt-1">
                {new Date(estimate.created_at).toLocaleDateString("ja-JP")}
              </p>
              <span className={`inline-block mt-2 text-xs font-medium px-2 py-0.5 rounded ${STATUS_COLOR[status]}`}>
                {STATUS_LABEL[status]}
              </span>
            </div>
          </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                <tr className="border-b border-gray-100">
                  <td className="py-3 pr-4 text-gray-800">見積合計（税抜）</td>
                  <td className="py-3 pl-4 text-right font-medium text-gray-800">{yen(estimate.subtotal)}</td>
                </tr>
              </tbody>
            </table>
          </div>
