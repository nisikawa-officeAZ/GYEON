...
// ── Scope discipline: the numbering allocator is NOT changed in R56B ─────────

test("the numbering allocator is untouched by this phase", () => {
  const code = codeOf(GATEWAY);
  assert.match(code, /getNextDocumentNumber\("estimate"\)/,
    "still calls the existing allocator — R56D corrects it, not R56B");
  const allocator = codeOf("src/lib/numbering/get-next-document-number.ts");
  assert.match(allocator, /getCurrentDealer\(\)/,
    "allocator still resolves the dealer independently (recorded R56D blocker)");
  assert.match(allocator, /fallbackNum/,
    "allocator still returns an unpersisted fallback number (recorded R56D blocker)");
});
