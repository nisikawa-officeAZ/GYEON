// InvoiceCustomerBlock / InvoiceVehicleBlock / InvoiceIssuerBlock — the 3-column party grid
// (01 Bill To / 02 Vehicle / 03 Issuer) from concept-b invoice. Built from shared primitives; issuer
// identity from BrandProfile. Same layout as the Estimate parties with invoice-specific labels.
...
import type { InvoiceCustomer, InvoiceVehicle } from "./invoice-data";
...
export function InvoiceCustomerBlock({ customer, accent }: { customer: InvoiceCustomer; accent: string }) {
  return (
    <Column>
      <PartyMarker num="01" en="Bill To" ja="ご請求先" accent={accent} />
...
export function InvoiceVehicleBlock({ vehicle, accent }: { vehicle: InvoiceVehicle; accent: string }) {
  return (
    <Column>
      <PartyMarker num="02" en="Vehicle" ja="対象車両" accent={accent} />
...
export function InvoiceIssuerBlock({ brand, accent }: { brand: BrandProfile; accent: string }) {
