// DeliveryNoteServiceTable — concept-b "Delivered Items" 6-column table:
// Category | Item & Description | Unit Price | Qty | Discount | Subtotal. Same structure as the
// Estimate/Invoice tables (shared primitives). Renders APPLIED pricing only — no recalculation.
...
import type { DeliveryItem } from "./delivery-note-data";
...
export function DeliveryNoteServiceTable({ items, accent }: { items: DeliveryItem[]; accent: string }) {
  return (
    <View style={{ marginBottom: 5 }}>
      <Row gap={6} style={{ alignItems: "center", marginBottom: 4 }}>
        <Value style={{ fontSize: FS.fs14, color: accent }}>04</Value>
        <Overline>Delivered Items</Overline>
        <Caption style={{ fontSize: FS.fs10 }}>納品項目</Caption>
        <View style={{ flex: 1 }} />
