// EstimateServiceTable — concept-b 6-column service table:
// Category | Item & Description | Unit Price | Qty | Discount | Subtotal.
// Category tag chip; rich item cell (name + description); numeric columns right-aligned; em-dash for
// no discount / unpriced lines (nothing false-reads as free).
...
import type { EstimateItem } from "./estimate-data";
...
export function EstimateServiceTable({ items, accent }: { items: EstimateItem[]; accent: string }) {
  return (
    <View style={{ marginBottom: 5 }}>
      {/* Section heading */}
      <Row gap={6} style={{ alignItems: "center", marginBottom: 4 }}>
        <Value style={{ fontSize: FS.fs14, color: accent }}>04</Value>
        <Overline>Services &amp; Products</Overline>
        <Caption style={{ fontSize: FS.fs10 }}>施工内容・製品</Caption>
        <View style={{ flex: 1 }} />
