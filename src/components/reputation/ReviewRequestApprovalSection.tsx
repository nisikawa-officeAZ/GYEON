...
//
// Sprint 11E Phase B/E: dealer-facing review request approval UI.
//
...
//   - Approve / Reject / Skip buttons (all dry-run in Sprint 11E)
//   - Draft message preview area (null until Phase 11F+ AI generation)
//   - Edit draft button (disabled — no draft yet)
//
// Persistence:
...
//
// LINE dispatch:
//   Not implemented. Deferred to Phase 11F+.
//
// AI generation:
//   Not implemented. Deferred to Phase 11F+.
...
export default function ReviewRequestApprovalSection({
  ...
  if (state.phase === "idle" || state.phase === "loading") {
    return (
      <p className="text-xs text-slate-500 text-center py-4 animate-pulse">
        レビュー依頼の準備状況を確認中...
      </p>
    );
  }
    ...
    if (!r.success) {
      return (
        <div className="flex flex-col gap-3">
          ...
          <button
            onClick={() => setState({ phase: "loaded", data, result: null, error: null })}
        ...
        {r.action === "approved" && (
          <div className="flex flex-col gap-1">
            <p className="text-[10px] text-amber-500/80">
              ⚠ LINE送信は未実装です（Phase 11F+で実装予定）
            </p>
            <p className="text-[10px] text-slate-600">
              ⚠ レビュー依頼はDBに保存されません（review_requestsテーブルのmigrationが必要）
            </p>
          </div>
        )}
        <button
    ...
    return (
      <div className="text-center py-3">
        <p className="text-xs text-slate-600">
          {data.missing_settings[0] ?? "顧客情報が必要です。"}
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-wrap items-center gap-2 pt-1">
        ...
        {/* Edit draft — disabled until AI generation is available */}
        <button
          disabled
          title="AIメッセージ生成後に有効化（Phase 11F+）"
      ...
      <p className="text-[10px] text-slate-700 border-t border-slate-800 pt-2">
        ※ 現在はドライランのみです。LINE送信 · AI生成 · DB保存は今後のPhaseで実装予定です。
      </p>
    </div>
  );
}
