...
test("2c. valid service + PPF null → service overrides plus default PPF", async () => {
  const cat = okCatalog(await resolveRow(VALID_SPC, null));
  assert.deepEqual(cat.ppfPlanPrices, DEFAULT_PRICING_CATALOG.ppfPlanPrices, "ppf tables stay default");
  assert.equal(cat.coatings.find((c) => c.id === "one-evo")?.base, 47000, "coating overridden");
  assert.deepEqual(cat, makePricingCatalog(dealerSettingsToPricingCatalog(VALID_SPC as unknown as ServicePriceSettings, null)));
});
...
test("3k. DEFAULT_SERVICE_PRICE_SETTINGS + DEFAULT_PPF_PRICE_TABLES resolve successfully", async () => {
  const cat = okCatalog(await resolveRow(DEFAULT_SERVICE_PRICE_SETTINGS, DEFAULT_PPF_PRICE_TABLES));
  assert.deepEqual(
    cat,
    makePricingCatalog(dealerSettingsToPricingCatalog(DEFAULT_SERVICE_PRICE_SETTINGS, DEFAULT_PPF_PRICE_TABLES)),
    "strict result matches legacy for the canonical defaults",
  );
});
...
test("4a. partial overrides keep every non-overridden canonical entry at its default", async () => {
  const cat = okCatalog(await resolveRow(VALID_SPC, VALID_PPF));
  assert.equal(cat.coatings.find((c) => c.id === "one-evo")?.base, 47000, "overridden");
  const def = DEFAULT_PRICING_CATALOG.coatings.find((c) => c.id === "pure-evo")?.base;
  assert.equal(cat.coatings.find((c) => c.id === "pure-evo")?.base, def, "non-overridden retains default");
});

test("4b. an explicit zero price is valid", async () => {
  const s = JSON.parse(JSON.stringify(VALID_SPC));
  s.coating.products[0].base_price_m = 0;
  const cat = okCatalog(await resolveRow(s, VALID_PPF));
  assert.equal(cat.coatings.find((c) => c.id === "one-evo")?.base, 0);
});

test("4c. every consumed pricing family is mapped exactly", async () => {
  const c = okCatalog(await resolveRow(VALID_SPC, VALID_PPF));
  assert.equal(c.coatings.find((x) => x.id === "one-evo")?.base, 47000);
  assert.equal(c.bodySizes.find((x) => x.key === "M")?.multi, 1.05);
  assert.equal(c.topcoatBase["one-evo"], 16000);
...
test("4d. a valid representative input matches legacy pricing output exactly", async () => {
  const strict = okCatalog(await resolveRow(VALID_SPC, VALID_PPF));
  const legacy = makePricingCatalog(
    dealerSettingsToPricingCatalog(VALID_SPC as unknown as ServicePriceSettings, VALID_PPF as unknown as PpfPriceTables),
  );
