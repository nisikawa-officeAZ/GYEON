/* B1B-E2-R1 — monthly-invoice pagination. Byte-derived from the accepted production
 * invoice-paginate.js (DOM-measurement pagination, fonts.ready gate, no URL parameters, no fixed
 * capacities). The estimate-single-page-fit / estimate-pagination-ready class names are the SHARED
 * contract used by the vendored CSS and the Chromium renderer ready-gate, so they are
...
(() => {
  ...
  const planPages = (rows, caps) => {
    ...
    if (total <= caps.first + caps.final) {
      /* Measurement-driven feasible-plan choice: among every split the MEASURED capacities
       * allow, take the fullest first page whose final page keeps at least two rows, so a
       * one-row orphan final page never happens when any feasible split avoids it. */
      let fallback = null;
      for (let first = Math.min(caps.first, total - 1); first >= 1; first -= 1) {
        const tail = total - first;
        if (tail > caps.final) continue;
        const split = [
          { continued: false, final: false, count: first },
          { continued: true, final: true, count: tail }
        ];
        if (tail >= 2) return split;
    ...
    /* Same orphan rule for 3+ page plans: when the MEASURED final capacity allows two rows,
     * pull one row back from the previous (now under-capacity) page instead of ending on one. */
    if (remaining === 1 && caps.final >= 2 && plan.length > 0 && plan[plan.length - 1].count >= 2) {
