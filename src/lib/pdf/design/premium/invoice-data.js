/* TEMPLATE-B3 production data binding for the native invoice template.
 *
 * Reads window.__DEALER_OS_DOC_CONTEXT__.documentData (built server-side by
 * invoice-document-context.ts from the persisted, snapshot-validated issuance state) and binds it
 * into the native invoice DOM BEFORE invoice-paginate.js measures pagination (script order
 * guarantees this listener registers, and therefore runs, first).
 *
 * Safety rules — identical to the accepted estimate binder:
...
(() => {
  const bind = () => {
    ...
    /* masthead meta: Issue Date / Payment Due (Currency stays as authored) */
    ...
    const dueMeta = metaByLabel('Payment Due');
    if (dueMeta) {
      if (d.paymentDueDisplay) dueMeta.querySelector('strong').textContent = d.paymentDueDisplay;
      else dueMeta.remove();
    } else problems.push('meta:Payment Due');
    ...
    /* 04 billing items — the single authored row is the structural template */
