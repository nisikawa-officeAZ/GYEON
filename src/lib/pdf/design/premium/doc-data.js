/* TEMPLATE-B2 production data binding for the accepted estimate template.
 *
 * Reads window.__DEALER_OS_DOC_CONTEXT__.documentData (built server-side by
 * estimate-document-context.ts from the persisted EstimateDB snapshot) and binds it into the
 * accepted DOM BEFORE estimate-paginate.js measures pagination (script order guarantees this
...
(() => {
  const bind = () => {
    ...
    /* masthead meta: Issue Date / Valid Until (Currency stays as authored) */
    ...
    const validMeta = metaByLabel('Valid Until');
    if (validMeta) {
      if (d.validUntilDisplay) validMeta.querySelector('strong').textContent = d.validUntilDisplay;
      else validMeta.remove();
    }
